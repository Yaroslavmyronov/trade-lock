...
export const MarketItemPrice = () => {
  const { selectedNfts } = useMarketSelectedNfts();
  const allNftPrices = selectedNfts.reduce((sum, nft) => sum + nft.price, 0);
  return (
    <div className="w-1/2 max-w-1/2 pl-5 text-[22px]">
      <div className="flex w-full flex-row items-center justify-end text-[#fff]">
        ...
        <div className="flex-end mx-[5px] flex text-[12px]">{`(${selectedNfts.length})`}</div>
